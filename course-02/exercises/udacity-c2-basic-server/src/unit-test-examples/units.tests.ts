...
describe('concat', () => {
  it('should concat 2 strings', () => {
    const result = concat('stringA', ' stringB')
    expect(result).to.equal('stringA stringB')
  })

  it('should throw an error if param is not passed', () => {
    expect(() => { concat(null, null) }).to.throw('empty string param')
  })
})
